    ...
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-emerald-50 via-green-50 to-white font-sans">
      ...
      <nav className="sticky top-0 z-50 flex items-center justify-between px-8 h-16 bg-white border-b border-gray-100 shadow-sm">
        ...
        <div className="flex items-center gap-1">
          <button className="px-4 py-2 rounded-lg text-sm font-medium text-gray-600 hover:bg-gray-100 transition">Dashboard</button>
          <button className="px-4 py-2 rounded-lg text-sm font-semibold bg-green-500 text-white">Explore</button>
          <button className="px-4 py-2 rounded-lg text-sm font-medium text-gray-600 hover:bg-gray-100 transition">Impact</button>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button className="w-9 h-9 rounded-xl bg-gray-200 text-gray-600 text-xs font-bold border-none">DB</button>
        </div>
      </nav>
      ...
      <div className="flex-1 flex flex-col items-center px-6 py-16">
        <div className="text-center mb-12">
          <h2 className="text-4xl font-extrabold text-slate-900 tracking-tight mb-3">Select your Role</h2>
          <p className="text-base text-slate-500">Pilih peran Anda untuk mulai perjalanan mengurangi food waste</p>
        </div>

        {error && (
          <div className="mb-6 px-4 py-3 bg-red-50 border border-red-200 text-red-600 text-sm rounded-xl w-full max-w-3xl">
            ⚠️ {error}
          </div>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-7 w-full max-w-3xl mb-12">
          ...
          <div onClick={() => setSelected("consumer")}
            ...
            <div className="p-7">
              ...
              <p className="text-sm text-slate-500 leading-relaxed mb-5">Dapatkan makanan berkualitas dengan harga hemat dan bantu kurangi pemborosan makanan</p>
              <ul className="space-y-2.5 mb-7">
                {["Hemat hingga 70%", "Pickup mudah & cepat", "Makanan berkualitas"].map((f) => (
                  <li key={f} className="flex items-center gap-2.5 text-sm font-semibold text-slate-700">
                    <span className="w-5 h-5 rounded-full border-2 border-green-500 flex items-center justify-center flex-shrink-0"><CheckIcon /></span>
                    {f}
                  </li>
                ))}
              </ul>
          ...
          <div onClick={() => setSelected("merchant")}
            ...
            <div className="p-7">
              ...
              <p className="text-sm text-slate-500 leading-relaxed mb-5">Jual makanan surplus dan kurangi kerugian sambil berkontribusi untuk lingkungan</p>
              <ul className="space-y-2.5 mb-7">
